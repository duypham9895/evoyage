...
eVoyage is built entirely by Codex (Anthropic's AI coding agent). Duy Phạm's role is Product Manager — defining features, making design decisions, and ensuring quality. This transparency is a core value of the project and should be reflected honestly in the UI.
...
|---|---|---|
| Test-driven development | `tdd` (Matt) | `superpowers:test-driven-development`, `everything-Codex:tdd`, `everything-Codex:tdd-workflow` |
...
For everything else not listed above (e.g. `gsd-*`, `everything-Codex:*` patterns/refs, `Codex-md-management:*`), use the existing skill — they don't conflict with Matt's set.
...
Apply [.Codex/skills/karpathy-guidelines/SKILL.md](./.Codex/skills/karpathy-guidelines/SKILL.md) on every code change. Invoke the `karpathy-guidelines` skill before writing, reviewing, or refactoring code.
...
- [ ] `npm test` passes (all 1237+ tests green)
